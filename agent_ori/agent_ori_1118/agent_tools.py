...
# 新增：会议内容总结工具
def summarize_meeting(query: str) -> str:
    global meeting_transcript
...
def information_retrieval_rag(action_input: str) -> str:
    global meeting_transcript, llm, current_question
    ...
    # 获取所有匹配位置的上下文窗口
    context_windows = []
    for position, similarity, keyword in matches[:10]:  # 限制最多处理前10个匹配
        context, bounds = extract_context_window(meeting_transcript, position)
    ...
    # 构建 prompt
    prompt = f"""根据以下会议内容回答问题，回复字数在100字以内：
### 问题 ###
{current_question}
###
...
tools = [
    ...
    Tool(
        name="信息检索RAG",
        func=information_retrieval_rag,
        description=(
...
# llm选择
# 根据需要选择模型，提前加载
#llm = LocalQwenLLM(model_name="Qwen/Qwen2.5-7B-Instruct-GPTQ-Int4", streaming_callback=streaming_callback)
#llm = LocalQwenLLM(model_name="Qwen/Qwen2-Audio-7B-Instruct", streaming_callback=streaming_callback)
#llm = LocalQwenLLM(model_name="Qwen/Qwen2.5-14B-Instruct-GPTQ-Int4", streaming_callback=streaming_callback)
#llm = LocalQwenLLM(model_name="Qwen/Qwen2.5-72B-Instruct-GPTQ-Int8", streaming_callback=streaming_callback)
#llm = LocalQwenLLM(model_name="LinkSoul/LLaSM-Cllama2", streaming_callback=streaming_callback)
